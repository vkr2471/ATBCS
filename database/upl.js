...
const file = reader.readFile(
  "C:\\Users\\venka\\Downloads\\Schedules_Domestic.xlsx"
);
...
data.forEach(async (res) => {
  ...
  let dayeff = [];
  let hihe = res["FREQUENCY"].split(",");
  hihe.forEach((meon) => {
    dayeff.push(days[meon]);
  });
  ...
  if (1 <= z && z < 2.5) cost = Math.floor(z * (Math.random() * 300 + 1300));
    ...
    cost = Math.floor(z * (Math.random() * 300 + 1600));
  else if (5 <= z && z < 7.5)
    cost = Math.floor(z * (Math.random() * 300 + 1900));
  const flight1 = {
    flightid: res["FLIGHT NO"]
      .replace(" ", "")
      .replace(" ", "0")
      .replace("SG", "KB"),
    source: res["ORIGIN"],
    ...
    arrival: res["ARRIVAL"],
    totalseats: { fc: meow.seats.fc, bc: meow.seats.bc, ec: meow.seats.ec },
    seatsbooked: { fc: 0, bc: 0, ec: 0 },
    ticketfare: { fc: cost * 24, bc: cost * 8, ec: cost },
  };
  while (now <= end) {
    var newdate = now.toISOString().split("T")[0];
    //console.log(newdate);
    if (dayeff.includes(now.getDay()) || dayeff.includes(8)) {
